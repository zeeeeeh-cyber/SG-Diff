...
def parse_args():
    parser = argparse.ArgumentParser(description="SG-Diff Base Model pre-training script.")
    parser.add_argument(
...
def main():
    ...
    logger.info("Initializing the SG-Diff UNet from the pretrained UNet.")
    in_channels = 14
    ...
    with torch.no_grad():
        ...
        old_in_channels = unet.conv_in.in_channels
        new_conv_in.weight[:, :old_in_channels, :, :].copy_(unet.conv_in.weight)
        new_conv_in.bias.copy_(unet.conv_in.bias)
        unet.conv_in = new_conv_in

    if args.freeze_backbone:
        logger.info("Freezing UNet backbone. Only `conv_in` will be trained.")
    ...
    def preprocess_train(examples):
        ...
        if args.prior_dir is not None:
            ...
            for img in examples[original_image_column]:
                if hasattr(img, "filename") and isinstance(img.filename, str):
                    base_name = os.path.basename(img.filename)
                elif isinstance(img, str):
                    base_name = os.path.basename(img)
                else:
                    raise ValueError(f"Cannot derive filename prefix from image type {type(img)}")
